...
class Controls:
  ...
  def update_events(self, CS):
    ...
    if (not self.sm.all_checks() or self.can_rcv_timeout) and no_system_errors and CS.canValid and not CS.canTimeout:
  ...
  def state_transition(self, CS):
    ...
    if self.state != State.disabled:
      ...
      elif self.CP.enhancedScc and CS.stockAeb:
        self.state = State.disabled
  ...
  def state_control(self, CS):
    ...
    CC.longActive = self.active and CS.cruiseState.enabled and not CS.brakePressed and not self.events.any(ET.OVERRIDE) and not self.events.any(ET.NO_ENTRY)
