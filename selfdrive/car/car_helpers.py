...
def fingerprint(logcan, sendcan):
  fixed_fingerprint = os.environ.get('FINGERPRINT', "") or (Params().get("CarModel") is not None)
  skip_fw_query = os.environ.get('SKIP_FW_QUERY', False) or \
                  (Params().get_bool("ToyotaForceSnG") and (Params().get("CarParamsCacheSnG") is not None) and
                  (Params().get("CarVinSnG") is not None))
  ecu_rx_addrs = set()
