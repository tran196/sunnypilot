...
class CarController:
  ...
  def update(self, CC, CS):
    ...
    if self.CP.carFingerprint in CANFD_CAR:
      ...
      if self.CP.openpilotLongitudinalControl and not enhanced_scc:
        if self.frame % 100 == 0:
      ...
      if self.frame % 2 == 0 and self.CP.openpilotLongitudinalControl:
        ...
        can_sends.extend(hyundaican.create_acc_commands(self.packer, CC.enabled and CS.out.cruiseState.enabled and CS.mainEnabled, accel, jerk, int(self.frame / 2),
                                                          hud_control.leadVisible, set_speed_in_units, stopping, CS.out.gasPressed, CS.mainEnabled, enhanced_scc, tau_gap_set))
        self.accel = accel
