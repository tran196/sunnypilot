  ...
  def update(self, CC, CS):
    ...
    if self.CP.carFingerprint in CANFD_CAR:
      ...
      if self.frame % 2 == 0 and not (self.CP.flags & HyundaiFlags.CANFD_HDA2):
        can_sends.append(hyundaicanfd.create_lfahda_cluster(self.packer, CC.enabled and CS.out.cruiseState.enabled, CC.latActive, CS.madsEnabled))
      ...
      if self.frame % 2 == 0 and self.CP.openpilotLongitudinalControl:
        ...
        starting = CS.out.vEgo < self.CP.vEgoStarting and accel > 0.0
        ...
        can_sends.extend(hyundaican.create_acc_commands(self.packer, CC.enabled and CS.out.cruiseState.enabled and CS.mainEnabled, accel, starting, int(self.frame / 2),
                                                          hud_control.leadVisible, set_speed_in_units, stopping, CS.out.gasPressed, CS.mainEnabled, self.CP.enhancedScc, tau_gap_set))
        self.accel = accel
      ...
      if self.frame % 50 == 0 and self.CP.openpilotLongitudinalControl and not self.CP.enhancedScc:
