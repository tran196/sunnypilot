    ...
    if ret.cruiseState.available:
      if not self.CP.pcmCruise:
        ...
        if self.gap_adjust_cruise:
          if self.gap_adjust_cruise_mode in (0, 2):
            if self.prev_cruise_buttons != 3 and self.cruise_buttons[-1] == 3:  # GAP
      ...
      if self.enable_mads:
        ...
        if self.cruise_buttons[-1] == 3:
          ...
          if self.mads_hold_counter > 50 and not self.mads_hold_gap:
            self.mads_hold_counter = 0
            self.mads_hold_gap = True
            self.madsEnabled = not self.madsEnabled
        else:
          self.mads_hold_counter = 0
          self.mads_hold_gap = False
